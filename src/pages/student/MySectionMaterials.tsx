...
import { academicApi, Topic, Material, Section } from '../../api/endpoints'
...
export function MySectionMaterials() {
  ...
  const { data: section, isLoading: loadingSection } = useQuery({
    queryKey: ['student-section', user?.section],
    queryFn: () => academicApi.getSections().then(res => 
      res.data.results.find((s: Section) => s.id === user?.section)
    ),
  ...
  if (!section) {
    return (
      <div className="text-center py-8">
        ...
        <h3 className="text-lg font-medium text-gray-900 mb-2">No hay sección asignada</h3>
        <p className="text-gray-500">No tienes una sección asignada. Contacta a tu administrador.</p>
      </div>
    )
  }
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-lg shadow p-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">Materiales de Mi Sección</h1>
            <p className="text-gray-600 mt-1">
              Sección: <span className="font-medium">{section.name}</span>
              {section.course && (
          ...
          <div className="text-right">
            <div className="text-sm text-gray-500">
              <div>Profesor: {section.professors_names?.join(', ') || 'No asignado'}</div>
              <div>Capacidad: {section.capacity} estudiantes</div>
            </div>
          </div>
        </div>
      </div>
